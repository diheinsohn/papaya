  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-8">
        <h1 className="text-2xl font-bold text-warm-900 mb-6">Mis ventas</h1>
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {items.length === 0 ? (
        <div className="text-center py-16">
          <p className="text-warm-500 text-lg">No tienes ventas aún</p>
        </div>
      ) : (
        <div className="space-y-4">
          {items.map((item) => (
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-2 mt-3">
                  {item.receipt_confirmed && !item.has_reviewed && (
                    <Link to={`/reviews/write/${item.listing.id}?role=seller`}>
                      <Button variant="outline" size="sm">Calificar comprador</Button>
                    </Link>
                  )}
                  {item.has_reviewed && (
